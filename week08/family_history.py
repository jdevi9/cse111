...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, value in marriages_dict.items():
        # pulls the key from the marriage dictionary to use in people dictionary
        husband_info_key = value[HUSBAND_KEY_INDEX]
        wife_info_key = value[WIFE_KEY_INDEX]
        husband_info = people_dict[husband_info_key]
        wife_info = people_dict[wife_info_key]
        # uses the pulled key to search for name in people dictionary
        husband_name = husband_info[NAME_INDEX]
        wife_name = wife_info[NAME_INDEX]
        # pulls needed information from picked out people's info
        husband_age_at_marriage = value[WEDDING_YEAR_INDEX] - \
            husband_info[BIRTH_YEAR_INDEX]
        wife_age_at_marriage = value[WEDDING_YEAR_INDEX] - \
            wife_info[BIRTH_YEAR_INDEX]
